...
代码位置：公共头 `services/log/log.h` 为 API 事实源；实现位于 services 层 log 模块（实现文件路径以符号 grep 定位，不在此列举）；经 selfreg 规则直接编译进 binary；**无初始化生命周期**（静态零初始化，无需 OM_INIT 注册）——**该属性仅限同步模式**：v2 异步模式引入 `OM_INIT_SERVICE` 建日志线程，"零 init"是同步模式的属性，不是服务永恒属性（见 ADR-0015 (log_service)）。
...
| 步骤 | v1 同步库（生产者上下文全包） | v2 异步队列（日志线程接管 ②③④⑤） | v3 混合（+ panic 直出旁路） |
|---|---|---|---|
| ① 提交 | 生产者（调用方） | 生产者（粗筛后入队，非阻塞） | 同 v2；调度器前 = memcpy 进早期缓冲 |
| ② 匹配 | 生产者（服务核心，临界区内） | 粗筛在生产者入队前、细筛在日志线程投递时 | 同 v2；panic 路径跳过（全出） |
| ③ 格式化 | 生产者（formatter） | 日志线程（时间戳此时统一加） | 日志线程；panic = 故障上下文直出 |
| ④ 投递扇出 | 生产者（服务核心） | 日志线程 | 同 v2；panic = 故障上下文直出 |
| ⑤ 后端输出 | 生产者（后端实现） | 日志线程 | 同 v2；panic = 故障上下文直出 |

跨版本不变量：**提交永远在生产者上下文**；**后端输出跟随投递的执行者**（后端实现不感知执行者是谁）；**匹配=决定谁收、投递=实际推送**（v1 两者连续于同一临界区，v2 拆开）。
