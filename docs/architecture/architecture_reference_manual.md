...
│                 systems  业务子系统                │
├──────────────────────────────────────────────────┤
│ services（通用服务） │ drivers（驱动与 PAL） │ boot（启动域） │
├──────────────────────────────────────────────────┤
...
**依赖管理原则**：模块化的目的是让每个模块**可独立修改、独立测试、独立替换**——决策的变化只影响与其相关的模块。按"**最可能变化的设计决策**"切分，接口只暴露不可变的抽象（信息隐蔽）。三条律：

| 定律 | 内容 |
|---|---|
| **无环律** | 依赖图有向无环——环使局部修改升级为全局协调、初始化顺序不可定（`OM_INIT_*` 分级即依赖拓扑的线性化） |
| **稳定方向律** | 依赖指向更不易变的一方：易变模块依赖稳定模块，反之即危险 |
| **抽象指向律** | 依赖指向抽象而非实现 |

**依赖规则**：层间依赖合法性 = 三律审查（无环 ∧ 指向稳定 ∧ 依赖抽象）。层位置是稳定方向排列的结果，非判据——"上层依赖下层"只是工程默认；抽象性与位置不符时以三律为准：抽象层可被反向依赖（实现依赖抽象合法）；跨层直连仅限抽象（禁止穿透抽象层连非抽象实现）。services 即稳定抽象（语义面向全体、接口稳定），只要无环即可服务任意层级。各层可依赖/禁止依赖清单见 §2。
...
### 2.4 领域模块 — drivers / services / boot
...
- **职责**：镜像格式契约（两工程共享的 ABI：头部布局、摘要区、判定顺序）、镜像解析与校验、启动决策状态机（读决策数据 → 判状态 → 校验 → 提交 → 启动/回退），以及决策数据的读写。提交策略（overwrite/swap/direct-xip/ram-load）由构建期固定，本层只承载策略无关的骨架。
- **边界**：平台无关、无业务语义，不承载下载/传输语义（那是服务与工程装配的事）；**不写镜像**——在只翻指针的提交策略下，本层只读镜像与读写决策数据。
- **介质访问**：经 `drivers` 的分区抽象按名进行；**槽 → 分区名的映射由装配方以数据提供**，本层代码中不出现分区名与地址字面量。
- **可依赖**：`drivers`（当前：storage 的分区抽象）、`algorithm`（摘要原语）、`kernel`。
- **禁止依赖**：`services`、`systems`、`platform`。
- **可被依赖**：`systems`（应用内的升级服务）、以及装配方（引导工程与带引导能力的应用工程）。
