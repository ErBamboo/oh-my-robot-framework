...
**依赖规则**：本框架的层级依赖是上述三律的具体化——**无环**（禁止下层反向依赖上层，反向即构成环）、**指向稳定**（依赖方向自上而下，上层可跨层直取下层）、**依赖抽象**（层级间以开放接口而非实现交互）。合并为一条总纲：

> **上层可依赖下层（可跨层直连）；禁止下层反向依赖上层；同层独立子系统互不依赖、禁止循环。**

**services 的通用性**：services 是对全体消费者开放的通用服务——语义面向全体、接口稳定（抽象而非实现）；只要不违反无环律（`services` 永不依赖 `drivers` 等下层即无环），可服务于任意层级：当前 `drivers` 可按服务逐个开放单向依赖（当前仅 `services/log/log.h`）；业务子系统之间仍互不依赖。

**层级依赖细则**：跨层——如上总纲；kernel 内部（core 定义/原语 · osal 抽象 · init 子系统）**允许互调**；由内部约定保证 `kernel-core` 子模块保持 OS 无关（不调用 osal），供主机侧测试（samples/host）；platform 层只依赖 `kernel`/`third_party`（见 §2.2），services 永不依赖 `drivers`（无环），drivers 可依赖 services 开放清单（如上）。
