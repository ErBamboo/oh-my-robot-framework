...
- toolchain：工具链名称，来自 [`oh-my-robot/xmake/toolchains/data.lua`](../../xmake/toolchains/data.lua)。
...
- [`oh-my-robot/xmake/xmake.lua`](../../xmake/xmake.lua)：OM 构建入口。
- [`oh-my-robot/xmake/config/`](../../xmake/config/)：构建选项定义与默认值。
- [`oh-my-robot/xmake/rules/`](../../xmake/rules/)：构建规则（上下文注入/板级资源/镜像转换）。
...
| `--toolchain` | 工具链名称 | [`oh-my-robot/xmake/toolchains/data.lua`](../../xmake/toolchains/data.lua) |
...
### 7.2 默认值优先级
- board / os：命令行 > 项目根 `om_preset.lua` > [`oh-my-robot/xmake/config/defaults.lua`](../../xmake/config/defaults.lua) > 数据目录按名称排序的第一个。
- sync_accel：命令行 > [`oh-my-robot/xmake/config/defaults.lua`](../../xmake/config/defaults.lua)（未配置时等价于 `auto`）。
- semihosting：命令行 > [`oh-my-robot/xmake/config/defaults.lua`](../../xmake/config/defaults.lua)（未配置时等价于 `off`）。
- toolchain：命令行 > 解析到的 preset 中 `toolchain_default.name` > 已保存配置 > [`oh-my-robot/xmake/toolchains/data.lua`](../../xmake/toolchains/data.lua) 中的 `default`。
...
- 默认值：`off`（定义于 [`oh-my-robot/xmake/config/defaults.lua`](../../xmake/config/defaults.lua)）。
- `semihosting=off`：
  - 构建系统会为 `binary` 目标自动注入 [`oh-my-robot/xmake/runtime/armclang/semihost_stub.c`](../../xmake/runtime/armclang/semihost_stub.c)。
