...
- **2026-09-16 追加（擦除归属实核后）**：三路调研（K-36..K-38）显示——① 生态中唯一"应用侧写状态**完全不擦除**"的成熟设计是 **MCUboot**（`image_ok` 单字节纯写，擦除归上传路径与 bootloader）；② 其余各家（ESP-IDF / nRF5 SDK / Gecko / TI）的应用侧写入都是"擦 + 写"；③ ST 官方 EEPROM 模拟（AN3969/AN2594）的"**预擦页 + 只 program**"机制，以及"**一个 program/erase cycle = 一次或多次写 + 一次页擦除**"的耐久定义，为本方案提供官方背书。据此**擦除归属改为引导侧**——写者只向已擦副本 program，bootloader 在引导期恢复不变式（候选 A 中"写前自擦另一份"的部分据此修订）。**代价**：不变式要求副本 **≥3 份**（2 份的失败推演见 `multi_strategy_boot_design.md` §5.2），**份数待讨论**。
...
- entry 置于**扇区头部**，每份独占一个 16K 扇区；**副本份数待讨论**（不变式要求 ≥3 份，第三份来源见 `multi_strategy_boot_design.md` §5.2：`meta` 32K + `meta_resv` 16K，或 bank2 头部 16K 空档，二者择一）。
- **读取**：CRC 有效且 `seq` 最大者胜；全部无效 → 冷启动路径（挑合法镜像，K-24/失败模式矩阵）。seq 比较用序号算术，`0xFFFFFFFF` 哨兵排除在外。
- **写序（2026-09-16 修订）**：**写者只向"已擦副本"program**（写 `0x00..0x1B` → **最后写 `crc32`**），**自身不做擦除**；**擦除归引导侧**——bootloader 在引导期把"非最新份"擦掉（只留最新份有效），恢复"至少一份已擦"的不变式。**永不擦最新有效份**；稳态（`state == VALID`）时 bootloader 零 flash 操作（R-SLOT-8 不破）。
...
- 擦除归属（2026-09-16 追加）：**"擦除与运行态不并发"为显式不变式**（RM0090 §3.6.5 明文禁止 write-while-write 且 BSY 为全局位）；应用运行期不做擦除。该性质**不依赖 RWW**——只依赖 NOR 通用语义 + "引导期无应用并发"，故在单 bank 器件上价值更大；RWW 只让"写 staging 镜像"一步在 F427 上免费。**未决**：副本份数（保持 2 份则每周期仍需一次应用侧 ≈260ms 擦除）。**边界**：ST 材料只对"寿命折算"背书（AN3969 §3.4.1："A program/erase cycle consists of one or more write accesses and one page erase operation"），**retention 无任何背书**——若保持性是硬指标，须保守折算或自做保持试验（K-38）。
